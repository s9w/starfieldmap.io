...
biome_names = {key:value["name"] for (key, value) in biomes_by_id.items()}
...
for biome in biomes_by_id.values():
    if "resgen_formid" in biome:
        ...
        res_by_biome[biome["name"]] = [resources_by_id[x]["name"] for x in resgen["res_formids"]]
...
#     "res_by_biome": res_by_biome
# }
with open("everything.json", "w") as out_f:
    json.dump(stars_for_galaxy, out_f, indent=2)
